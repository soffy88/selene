...
class BarData:
    symbol: str; interval: str; open_time: int
    ...
    @property
    def dt(self) -> datetime: return datetime.utcfromtimestamp(self.open_time / 1000)

    def to_dict(self) -> dict:
        return {"symbol": self.symbol, "interval": self.interval,
                "open_time": self.open_time, "open": self.open,
                "high": self.high, "low": self.low, "close": self.close, "volume": self.volume}


@dataclass(frozen=True)
class FundingRateData:
    symbol: str; funding_time: int; rate_pct: float; is_anomaly: bool = False
    @property
    def dt(self) -> datetime: return datetime.utcfromtimestamp(self.funding_time / 1000)
...
class OrderBook:
    ...
    @property
    def bid(self) -> float: return self.bids[0][0] if self.bids else 0.0
    @property
    def ask(self) -> float: return self.asks[0][0] if self.asks else 0.0
    @property
