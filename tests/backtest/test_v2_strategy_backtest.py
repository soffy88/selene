    ...
    df = pd.DataFrame(
        {
            ...
            "high": np.full(n, 30000.0),
            "low": np.full(n, 30000.0),
            "volume": 1000.0,
        }
    )
...
def test_default_trials_non_degenerate_and_deflates_dsr():
    ...
    trades = [
        _TradeView(
            exit_time=1000 + i * 86_400_000,
            pnl_usd=120.0 if i % 5 else -90.0,
            strategy="strategy_1",
            direction="LONG",
            entry_time_ms=0,
        )
...
def test_enforce_oos_gate_is_binding():
    ...
    with pytest.raises(BacktestRejected, match="FAILED"):
        enforce_oos_gate({"oos": {}, "oos_passed": False, "oos_fail_reasons": ["sharpe 0.20 < 1.0"]})
...
def test_check_pass_flags_weak_metrics():
    weak = evaluate_trades(
        [
            _TradeView(exit_time=1000 + i, pnl_usd=-10.0, strategy="strategy_1", direction="LONG", entry_time_ms=0)
            for i in range(5)
        ],
        initial_capital=100_000,
    )
