...
def _make_candles(n: int) -> list[dict]:
    ...
    for i in range(n):
        ...
        out.append({"open_time": BASE_MS + i * HOUR_MS, "high": close * 1.004,
                    "low": close * 0.996, "close": close, "volume": 1000.0})
    return out


def _make_funding(n: int) -> list[dict]:
    return [{"funding_time": BASE_MS + i * HOUR_MS,
             "funding_rate": -0.10 if (i // 12) % 2 == 0 else 0.15} for i in range(n)]
...
def _install_fake_oskill(monkeypatch, capture, kwargs_seen=None):
    ...
    def cpcv_pipeline(n_total, *, n_folds, n_test_groups, embargo_pct,
        ...
        return {
            "paths_sharpe_distribution": {"mean": float(arr.mean()), "std": float(arr.std())},
            "median_sharpe": float(np.median(arr)), "min_sharpe": float(arr.min()),
            "max_sharpe": float(arr.max()), "n_paths": len(arr),
        }
...
@pytest.mark.skipif(True, reason="real oskill not installed locally")
def test_real_oskill_cpcv_runs():  # pragma: no cover - CI-only with oskill
