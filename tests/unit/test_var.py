...
class TestHistoricalVaR:
    ...
    def test_fat_tail_var_higher_than_normal(self):
        ...
        fat_tail = [random.gauss(0,100) + (random.choice([-1,1])*500 if random.random()<0.05 else 0)
                    for _ in range(500)]
...
class TestDrawdownController:
    ...
    def test_manual_reset(self):
        dc = DrawdownController()
        dc.update(10000); dc.update(8000)  # RED
        assert dc.is_halted
        dc.manual_reset()
