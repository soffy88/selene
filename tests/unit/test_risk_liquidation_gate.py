...
def test_stop_beyond_liquidation_rejected(monkeypatch):
    ...
    monkeypatch.setattr(rm, "_open_positions",
                        {"BTCUSDT": {"side": "LONG", "notional": 70_000.0}}, raising=False)
    ok, reason = rm._gate.check_liquidation_distance(
...
def test_excessive_leverage_backstop_rejected(monkeypatch):
    ...
    monkeypatch.setattr(rm, "_open_positions",
                        {"BTCUSDT": {"side": "LONG", "notional": 110_000.0}}, raising=False)
    ok, reason = rm._gate.check_liquidation_distance(
...
def test_approve_blocks_stop_beyond_liquidation(monkeypatch):
    ...
    monkeypatch.setattr(rm, "_open_positions",
                        {"__equity__": {"value": 10_000.0},
                         "BTCUSDT": {"side": "LONG", "notional": 18_000.0}}, raising=False)
    ...
    order = {
        "symbol": "ETHUSDT", "side": "BUY", "allocated_usd": 8_000.0,
        "win_probability": 0.99, "quantity": 1.0,
        "entry_price": 100.0, "stop_price": 60.0,
    }
...
def test_approve_allows_tight_stop(monkeypatch):
    _set_corr_returns(monkeypatch, {})
    monkeypatch.setattr(rm, "_open_positions",
                        {"__equity__": {"value": 10_000.0}}, raising=False)
    order = {
        "symbol": "BTCUSDT", "side": "BUY", "allocated_usd": 5_000.0,
        "win_probability": 0.99, "quantity": 1.0,
