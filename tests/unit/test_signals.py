...
class TestScoredSignal:
    def test_is_actionable_passes(self):
        s = ScoredSignal(symbol="BTCUSDT", win_probability=0.62, data_quality=0.95, entry_price=50000, stop_loss=49000)
        assert s.is_actionable

    def test_is_not_actionable_low_prob(self):
        s = ScoredSignal(symbol="BTCUSDT", win_probability=0.50, data_quality=0.95, entry_price=50000, stop_loss=49000)
        assert not s.is_actionable

    def test_is_not_actionable_low_quality(self):
        s = ScoredSignal(symbol="BTCUSDT", win_probability=0.65, data_quality=0.70, entry_price=50000, stop_loss=49000)
        assert not s.is_actionable

    def test_risk_reward_calculation(self):
        s = ScoredSignal(entry_price=100, stop_loss=98, take_profit=106, direction=Direction.LONG)
