...
from services.signal.regime.detector import REGIME_PARAMS, RegimeDetector, _calc_adx, _calc_atr
...
class TestADX:
    ...
    def test_adx_in_bounds(self):
        ...
        for _ in range(5):
            ...
            low = [x - random.uniform(0, 1) for x in c]
            adx = _calc_adx(h, low, c)
            if adx:
                assert 0 <= adx <= 100
