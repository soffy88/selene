...
class OrderState(str, Enum):
    QUEUED="QUEUED"; SLIPPAGE_ESTIMATE="SLIPPAGE_ESTIMATE"; ROUTING="ROUTING"
    SUBMITTING="SUBMITTING"; PENDING_ACK="PENDING_ACK"; OPEN="OPEN"
    PARTIALLY_FILLED="PARTIALLY_FILLED"; FILLED="FILLED"; MONITORING="MONITORING"
    CLOSING="CLOSING"; CLOSED="CLOSED"; CANCELLED="CANCELLED"; FAILED="FAILED"

VALID_TRANSITIONS = {
    OrderState.QUEUED:            {OrderState.SLIPPAGE_ESTIMATE:"start", OrderState.CANCELLED:"timeout"},
    OrderState.SLIPPAGE_ESTIMATE: {OrderState.ROUTING:"ok", OrderState.QUEUED:"requeue", OrderState.CANCELLED:"too high"},
    ...
    OrderState.OPEN:              {OrderState.PARTIALLY_FILLED:"partial", OrderState.FILLED:"full", OrderState.CANCELLED:"cancel"},
    OrderState.PARTIALLY_FILLED: {OrderState.FILLED:"complete", OrderState.CANCELLED:"cancel", OrderState.MONITORING:"accept partial"},
...
class FSM:
    def __init__(self): self.state = OrderState.QUEUED; self.history = []
...
class TestOrderFSM:
    def test_happy_path_market_order(self):
        fsm = FSM()
        for state in [
            OrderState.SLIPPAGE_ESTIMATE, OrderState.ROUTING,
            OrderState.SUBMITTING, OrderState.PENDING_ACK,
            OrderState.OPEN, OrderState.FILLED,
            OrderState.MONITORING, OrderState.CLOSING, OrderState.CLOSED,
        ]:
