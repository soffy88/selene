...
class TestIndividualFactors:
    ...
    def test_all_factors_bounded(self):
        ...
        scores = [
            ...
            score_oi_momentum(10, 5),
            score_oi_momentum(-10, -5),
            score_lsr_divergence(75),
            score_lsr_divergence(25),
        ]
        for s in scores:
...
class TestWeights:
    ...
    def test_funding_highest_weight(self):
        ...
        assert WEIGHTS["funding_zscore"] >= max(v for k, v in WEIGHTS.items() if k != "funding_zscore")
...
class TestCompositeScorer:
    ...
    def test_probability_in_valid_range(self):
        ...
        random.seed(0)
        for _ in range(100):
            factors = FactorScores(**{k: random.uniform(-1, 1) for k in vars(FactorScores())})
