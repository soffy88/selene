...
def _open(
    account: SubAccount, price: float = 50_000.0, direction: str = "LONG", size_pct: float = 0.20, leverage: float = 1.0
):
...
def test_close_profitable_position_updates_nav():
    ...
    closed = e.subaccount_1.close_position(pos.id, exit_price=55_000.0, exit_time=_NOW, exit_reason="test")
...
def test_close_losing_position_reduces_nav():
    ...
    closed = e.subaccount_1.close_position(pos.id, exit_price=48_500.0, exit_time=_NOW, exit_reason="drawdown")
    assert closed is not None
...
def test_cascade_closes_all_strategy2_positions():
    ...
    _open(e.subaccount_2, size_pct=0.10, direction="SHORT")
...
def test_unrealized_pnl_long():
    e = _engine()
    _open(e.subaccount_1, price=50_000.0, size_pct=0.20)
...
def test_unrealized_pnl_short():
    e = _engine()
    _open(e.subaccount_1, price=50_000.0, size_pct=0.20, direction="SHORT")
