    ...
    async def fetch(self, query, *args):
        if args:
            return [{"param_key": k, "param_value": self._store[k]}
                    for k in args[0] if k in self._store]
...
def test_tda_persist_writes_live_p95_key():
    ...
    assert persisted == {"l1_threshold_p90": 1.0e-4, "l1_threshold_p95": 2.0e-4,
                         "l1_threshold_p97": 3.0e-4}
    assert set(store) == {"tda1_l1_threshold_p90", "tda1_l1_threshold_p95",
...
def test_tda_persist_skips_non_finite():
    store: dict = {}
    with _patch_inmemory(store):
        out = tda_persist({"l1_q90": 1e-4, "l1_q95": float("nan"), "l1_q97": 3e-4},
...
def test_hawkes_persist_accepts_subcritical_fit():
    store: dict = {}
    with _patch_inmemory(store):
        out = hawkes_persist({"mu": 0.09, "alpha": 0.024, "beta": 0.043},
                             db_url="postgresql://t:t@localhost/t")
    assert out["mu_ref"] == 0.09
    assert set(store) == {"h2_mu_ref", "h2_alpha_ref", "h2_beta_ref",
...
def test_expected_keys_cover_live_readers():
    ...
    for k in ("h2_mu_ref", "h2_alpha_ref", "h2_beta_ref",
              "h2_branching_ratio_threshold", "tda1_l1_threshold_p95"):
...
async def test_export_bars_parquet(tmp_path):
    rows = [{"time": datetime(2026, 6, 1, h, tzinfo=timezone.utc),
             "open": 100.0 + h, "high": 101.0 + h, "low": 99.0 + h,
             "close": 100.5 + h, "volume": 10.0} for h in (0, 4, 8)]
    path = str(tmp_path / "bars.parquet")
    n = await calibrate_all.export_bars_parquet(_FakePool(_FakeConn(fetch_rows=rows)),
                                                path, symbol="BTC-USDT")
    assert n == 3
    import pandas as pd
