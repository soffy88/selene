...
from sel_v2.states.transitions import identify_transition, get_legal_exits, _LEGAL_TRANSITIONS
...
@pytest.mark.parametrize("from_s, to_s, expected_vocab", [
...
def test_recognizer_assigns_transition_via():
    ...
    def _feat(sigma_pctile, sigma_monotone=None, hawkes_br=None,
              tda_pctile=None, tda_monotone=None, bar_index=600):
        return BarFeatures(
            timestamp=_TS, bar_index=bar_index,
            close=50000.0, log_price=10.8,
            sigma_4h=0.01, sigma_pctile=sigma_pctile,
            sigma_monotone_3bar=sigma_monotone,
            price_breakout_up=False, price_breakout_down=False,
            hawkes_br=hawkes_br, hawkes_br_threshold=0.85,
            tda_l1=None, tda_l1_pctile=tda_pctile,
            tda_l1_threshold=0.000097, tda_l1_monotone_3bar=tda_monotone,
            cold_start=False,
        )

    # Stay in Coiling for 6 bars (min_dwell=6)
    for i in range(6):
        r = recognizer.recognize(_feat(sigma_pctile=0.20, bar_index=i))

    # Now transition to Critical (should produce "Stress")
    r_critical = recognizer.recognize(
        _feat(sigma_pctile=0.96, sigma_monotone=True,
              hawkes_br=0.90, tda_pctile=0.98, tda_monotone=True, bar_index=7)
    )
