...
def test_h1_gate_passes_high_intensity():
    f = Strategy2EntryFilter()
    f.hawkes_tracker = HawkesIntensityTracker(HawkesParams(mu=100.0, alpha=0.3, beta=0.5))
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(
        t=1e6, cusum_trigger=trigger, state_4h="Coiling", inverse_vocab=["Sweep"], ofi_persistent_same_direction=True
    )
...
def test_cascade_state_aborts():
    f = Strategy2EntryFilter()
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(t=1e6, cusum_trigger=trigger, state_4h="Cascade", inverse_vocab=["Sweep"])
...
def test_critical_state_allowed():
    ...
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(
        t=1e6, cusum_trigger=trigger, state_4h="Critical", inverse_vocab=["Sweep"], ofi_persistent_same_direction=True
    )
...
def test_empty_vocab_aborts():
    ...
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(
        t=1e6, cusum_trigger=trigger, state_4h="Coiling", inverse_vocab=[], ofi_persistent_same_direction=True
    )
    assert d.action == "ABORT"
    assert d.step_reached == 3
...
def test_type_a_reversal_inverts_direction():
    ...
    f = Strategy2EntryFilter()
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(t=1e6, cusum_trigger=trigger, state_4h="Coiling", inverse_vocab=["Absorption", "Sweep"])
...
def test_entry_size_scales_with_cusum_intensity():
    ...
    weak = f.evaluate(
        t=1e6,
        cusum_trigger=_make_trigger(direction="LONG", coeff=1.0),
        state_4h="Coiling",
        inverse_vocab=["Sweep"],
        ofi_persistent_same_direction=True,
    )
    ...
    strong = f2.evaluate(
        t=1e6,
        cusum_trigger=_make_trigger(direction="LONG", coeff=2.5),
        state_4h="Coiling",
        inverse_vocab=["Sweep"],
        ofi_persistent_same_direction=True,
    )
...
def test_type_b_aborts_when_ofi_unknown():
    ...
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(
        t=1e6, cusum_trigger=trigger, state_4h="Coiling", inverse_vocab=["Sweep"], ofi_persistent_same_direction=None
    )
    assert d.action == "ABORT"
    assert d.step_reached == 3
...
def test_type_b_aborts_when_ofi_false():
    ...
    trigger = _make_trigger(triggered=True, direction="LONG")
    d = f.evaluate(
        t=1e6, cusum_trigger=trigger, state_4h="Coiling", inverse_vocab=["Sweep"], ofi_persistent_same_direction=False
    )
    assert d.action == "ABORT"
    assert d.step_reached == 3
