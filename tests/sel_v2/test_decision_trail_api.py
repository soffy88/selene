    ...
    rows = [
        {
            ...
            "state": "Surging",
            "transition_from": "Cascade",
            "transition_via": "Exhaustion",
            "duration_4h": 3,
            "state_features": {
                "a": 1,
                "b": None,
...
def test_sel_chart_returns_ascending_ohlc_with_state(monkeypatch):
    ...
    rows = [  # DESC order as the SQL returns
        {
            "time": t2,
            "open": decimal.Decimal("2"),
            "high": decimal.Decimal("3"),
            "low": decimal.Decimal("1"),
            "close": decimal.Decimal("2.5"),
            "state": "Surging",
        },
        {
            "time": t1,
...
def test_decision_view_flattens_entry_decision():
    ...
    class _D:
        action = "OBSERVE"
        reason = "Step 1b: Hawkes λ* below threshold"
        step_reached = 2
        direction = None
...
def test_write_decision_trail_bulk_upserts(monkeypatch):
    ...
    rows = [
        (
            ...
            "Surging",
            None,
            {"z_t": 0.1, "entropy_variance_rising": True},
        )
    ]
