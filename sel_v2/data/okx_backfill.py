...
def main():
    ...
    parser.add_argument("--symbol", default="BTC-USDT", help="stored symbol (base) — downstream joins on this")
    parser.add_argument(
        "--inst-id",
        default=None,
    ...
    while end_ts > start_ts:
        ...
        for c in candles:
            ...
            """,
                (ts, args.symbol, o, h, low, cl, vol, vwap, tick_count),
            )
            total_inserted += 1
        ...
        print(
            f"Inserted {len(candles)} bars, total {total_inserted}, min_ts: {datetime.fromtimestamp(end_ts / 1000, tz=timezone.utc)}"
        )
        time.sleep(0.2)
