...
def te_pvalue(
    ...
    surr_tes = [symbolic_te(phase_randomize(source, rng), target, d=d, lag=lag) for _ in range(n_surrogates)]
...
def build_signals(df: pd.DataFrame) -> dict[str, np.ndarray]:
    ...
    window = 24
    rv = np.array([np.std(ret[max(0, i - window) : i]) if i >= 2 else np.nan for i in range(1, n)])
...
def main() -> None:
    ...
    parser.add_argument("--d", type=int, default=3, help="Ordinal pattern order")
