...
class OKXAdapter(BaseAdapter):
    ...
    def _sign(self, ts: str, method: str, path: str, body: str = "") -> str:
        ...
        return base64.b64encode(hmac.new(self.api_secret.encode(), msg.encode(), hashlib.sha256).digest()).decode()
    ...
    async def place_order(
        ...
        if code != "0":
            ...
            if cl_ord_id and _is_duplicate_order(s_code, err):
                ...
                if recovered:
                    return recovered
            logger.error(
                f"OKX order error code={code} msg={data.get('msg')} sCode={s_code} sMsg={s_msg} payload={payload}"
            )
        ...
        if order.get("sCode", "0") != "0":
            ...
            return OrderResult(
                success=False, error=order.get("sMsg", f"sCode={order['sCode']}"), client_order_id=cl_ord_id
            )
        return OrderResult(
            success=True,
    ...
    async def place_stop_order(
        ...
        if order.get("sCode", "0") != "0":
            return OrderResult(
                success=False, error=order.get("sMsg", f"sCode={order.get('sCode')}"), client_order_id=cl_ord_id
            )
        return OrderResult(
            success=True,
            ...
            try:
                async with websockets.connect(self._ws_url, ping_interval=20) as ws:
                    ...
                    sig = base64.b64encode(
                        hmac.new(
                            self.api_secret.encode(), (ts + "GET" + "/users/self/verify").encode(), hashlib.sha256
                        ).digest()
                    ).decode()
                    await ws.send(
                        json.dumps(
