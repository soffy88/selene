...
from services.execution.adapters.base import get_all_adapters
...
class SmartRouter:
    ...
    async def route(
        ...
        # 并发查询所有交易所盘口
        tasks = {
            name: asyncio.create_task(adapter.get_orderbook(symbol, depth=20)) for name, adapter in adapters.items()
        }
        books = {}
    ...
    def _analyze_book(
        ...
        try:
            ...
            fee_rate = FEES.get(name, {}).get("maker" if order_type == "LIMIT" else "taker", 0.0004)
