...
def generate_recommendation(
    ...
    can_confirm = (
        mean_ic is not None and mean_ic >= T["ic_min_for_confirm"] and
        (mean_ir is None or mean_ir >= T["ir_min_for_confirm"]) and
        avg_prob >= T["win_prob_avg_min"] and
        T["signal_freq_min"] <= per_day <= T["signal_freq_max"] and
        dd_pct <= T["drawdown_max_confirm"]
    )

    # 判断是否可以从 CONFIRM → AUTO
    can_auto = (
        can_confirm and
        mean_ic >= T["ic_min_for_auto"] and
        (mean_ir is None or mean_ir >= T["ir_min_for_auto"]) and
        dd_pct <= T["drawdown_max_auto"] and
        fill_rate is not None and fill_rate >= 0.90 and
        total_n >= 50
    )
...
╚{'═'*50}╝""")
    ...
    if ic["status"] == "no_data":
        ...
        ic_status = (
            "🔴 信号接近随机" if mean_ic is not None and mean_ic < 0.03 else
            "🟡 信号弱，需观察" if mean_ic is not None and mean_ic < 0.05 else
            "🟢 信号有效" if mean_ic is not None else "⏳ 计算中"
        )
    ...
    if signals["status"] == "no_data":
        ...
        freq_warn = (
            "⚠ 过多，检查过拟合" if signals["per_day"] > THRESHOLDS["signal_freq_max"] else
            "⚠ 过少，策略过严" if signals["per_day"] < THRESHOLDS["signal_freq_min"] else ""
        )
    ...
    if regimes["status"] == "no_data":
        ...
        row("稳定性判断", stability)
        row("高波动占比", f"{regimes['high_vol_pct']:.0%}",
            "当前处于高波动，策略受限" if regimes["high_vol_pct"] > 0.3 else "")
        row("UNKNOWN 占比", f"{regimes['unknown_pct']:.0%}",
            "数据量不足" if regimes["unknown_pct"] > 0.2 else "")
        lines.append("")
    ...
    if not risk:
        ...
        row("当前回撤", f"{dd:.2f}%",
            "超过切换阈值" if dd/100 > THRESHOLDS["drawdown_max_confirm"] else "")
...
def run(days: int = 1, push: bool = False, current_mode: str = "NOTIFY_ONLY") -> dict:
    ...
    rec = generate_recommendation(
        ic_analysis, signal_analysis, regime_analysis,
        risk_raw, order_analysis, current_mode,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--push",  action="store_true",
                        help="推送到 Telegram")
    parser.add_argument("--mode",  default=os.environ.get("EXEC_MODE", "NOTIFY_ONLY"),
